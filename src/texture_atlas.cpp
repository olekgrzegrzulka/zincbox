...
TextureAtlas::TextureAtlas() {
  ...
  free_16x16_squares.resize((atlas_size / square_size_px) * (atlas_size / square_size_px), true);
}
...
std::optional<std::pair<i32, i32>> TextureAtlas::find_space_for_texture(i32 width, i32 height) {
  for (i32 square_x = 0; square_x < (atlas_size / square_size_px); square_x += 1) {
    for (i32 square_y = 0; square_y < (atlas_size / square_size_px); square_y += 1) {
      bool fail = false;
      for (i32 square_width = 0; square_width < std::ceil(width / (double)square_size_px); square_width += 1) {
        for (i32 square_height = 0; square_height < std::ceil(height / (double)square_size_px); square_height += 1) {
          size_t square_i = (atlas_size / square_size_px) * (square_y + square_height) + (square_x + square_width);
          // size_t square_i = square_x + square_y * (atlas_size / square_size_px);
          if (square_x + square_width >= (atlas_size / square_size_px) || square_y + square_height >= (atlas_size / square_size_px)) {
            fail = true;
            break;
          }
      ...
      if (!fail) {
        return std::make_pair(square_x * square_size_px, square_y * square_size_px);
      }
    }
  }
  debug_warn("fail");
...
void TextureAtlas::mark_space_for_texture(i32 x, i32 y, i32 width, i32 height) {
  i32 begin_x = x / square_size_px;
  i32 begin_y = y / square_size_px;
  i32 end_x = begin_x + std::ceil(width / (double)square_size_px);
  i32 end_y = begin_y + std::ceil(height / (double)square_size_px);

  for (i32 square_x = begin_x; square_x < end_x; square_x += 1) {
    for (i32 square_y = begin_y; square_y < end_y; square_y += 1) {
      // size_t square_i = square_x + square_y * (atlas_size / square_size_px);
      ensure(free_16x16_squares[(atlas_size / square_size_px) * square_y + square_x]);
      // ensure(free_16x16_squares[square_i]);
      free_16x16_squares[(atlas_size / square_size_px) * square_y + square_x] = false;
