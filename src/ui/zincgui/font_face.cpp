...
zincgui::FontFace::FontFace(FT_Library& freetype_lib, const std::string& path, i32 pixel_height) {
...
zincgui::FontFace::FontFace(FT_Library& freetype_lib, void* data, size_t data_size, i32 pixel_height) {
  FT_Face freetype_face;
...
bool zincgui::FontFace::init(FT_Face& face, i32 pixel_height) {
  bool success =
 ...
 */
void zincgui::FontFace::bind(u32 slot) const {
  ensure(texture != 0);
...
const zincgui::FontGlyph* zincgui::FontFace::find_glyph(u32 charcode) const {
  auto it = glyph_map.find(charcode);
  if (it == glyph_map.end()) { return nullptr; }
  return &it->second;
}

bool zincgui::FontFace::try_creating_glyph_data(FT_Face& freetype_face, i32 texture_dimensions, i32 pixel_height) {
