...
namespace zincgui {
  ...
  class Button : public Sprite {
    ...
    public:
      Button(Root& ui_) : Sprite::Sprite(ui_), label(add_child<Label>()) { init(); }

      Button(Root& ui_, std::string_view label_) : Sprite::Sprite(ui_), label(add_child<Label>(label_)) { init(); }
