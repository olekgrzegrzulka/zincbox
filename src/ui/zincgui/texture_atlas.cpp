...
bool zincgui::TextureAtlas::add_texture(std::string_view id, const std::filesystem::path& path) {
...
bool zincgui::TextureAtlas::add_texture(std::string_view id, const std::vector<u8>& data_, i32 width, i32 height) {
  if (textures.contains(id)) { return false; }
...
bool zincgui::TextureAtlas::add_texture(std::string_view id, const u8* data_, i32 width, i32 height) {
  if (textures.contains(id)) { return false; }
...
bool zincgui::TextureAtlas::add_texture_row(std::span<const std::string> ids, const std::filesystem::path& path) {
...
bool zincgui::TextureAtlas::add_texture_row(std::span<const std::string> ids, const u8* data, i32 width, i32 height) {
  for (const auto& id : ids) {
    if (textures.contains(id)) { return false; }
  }
...
void zincgui::TextureAtlas::add_texture_alias(std::string id, std::string to) {
  aliases[std::move(id)] = std::move(to);
}

bool zincgui::TextureAtlas::remove_texture(std::string_view id) {
...
vec2i zincgui::TextureAtlas::get_texture_size(std::string_view id) {
...
void zincgui::TextureAtlas::save_to_file(std::string filename) {
...
std::optional<std::reference_wrapper<zincgui::TextureAtlasData>>
zincgui::TextureAtlas::get_internal(std::string_view id, i32 max_depth) {
  if (max_depth <= 0) {
...
void zincgui::TextureAtlas::bind(u32 slot) {
...
void zincgui::TextureAtlas::set_fallback_texture(std::string_view id) {
  if (!has_texture(id)) { return; }
  fallback_texture = id;
}

void zincgui::TextureAtlas::regenerate_texture() {
...
std::optional<std::pair<i32, i32>> zincgui::TextureAtlas::find_space_for_texture(i32 width, i32 height) {
...
void zincgui::TextureAtlas::mark_space_for_texture(i32 x, i32 y, i32 width, i32 height, bool unmark) {
...
vec2i zincgui::TextureAtlas::paste_texture(stbi_uc* data, i32 width, i32 height) {
